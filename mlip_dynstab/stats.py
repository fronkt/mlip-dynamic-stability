...
__all__ = [
    ...
    "phi", "cluster_permutation_phi", "spearman", "cluster_permutation_spearman",
]
...
def cluster_permutation_phi(H: np.ndarray, F: np.ndarray, n_perm: int = 10000,
    ...
    Lifted unchanged in behaviour from ``scripts/estimator_noise.py`` so the published numbers
    are reproduced to the digit; that script now calls this.
    """
    H = np.asarray(H, bool)
    F = np.asarray(F, bool)
    n_sys = H.shape[0]
